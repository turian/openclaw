...
| `dockerCommand`       | Overrides Dockerfile CMD to bind publicly and listen on `$PORT` |
...
| `generateValue: true` | Auto-generates a secure token (required for public bind)   |
| `disk`                | Persistent storage that survives redeploys                 |

## Choosing a plan

| Plan     | RAM  | Price  | Disk          | Best for                      |
| -------- | ---- | ------ | ------------- | ----------------------------- |
| Free     | 512 MB | $0/mo  | Not available | Testing only (OOM likely)     |
| Starter  | 512 MB | $7/mo  | 1GB+          | Not recommended (OOM likely)  |
| Standard | 2 GB | $25/mo | 1GB+          | Recommended minimum           |
| Pro      | 4 GB | $85/mo | 1GB+          | Multiple channels, heavy use  |

The Blueprint defaults to `standard` (2 GB RAM). The gateway requires roughly 550 MB at
startup, so plans with 512 MB or less will fail with out-of-memory errors.
...
- OOM on Starter/Free plans — upgrade to Standard (2 GB RAM minimum)

### Slow cold starts (free tier)

Free tier services spin down after 15 minutes of inactivity. The first request after spin-down takes a few seconds while the container starts. Upgrade to Standard plan for always-on.
